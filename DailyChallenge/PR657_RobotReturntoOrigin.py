...
class Solution:
    def judgeCircle(self, moves: str) -> bool:
        """
        Determine whether a robot returns to the origin (0, 0) after executing a sequence of moves.

        Problem Context:
        ----------------
        - The robot starts at the origin point (0, 0) on a 2D plane.
        - Moves are represented by characters:
            'R' → move right (x + 1)
            'L' → move left  (x - 1)
            'U' → move up    (y + 1)
            'D' → move down  (y - 1)
        - After executing all moves, check if the robot is back at the origin.

        Parameters:
        -----------
        moves : str
            A string consisting of 'R', 'L', 'U', 'D' characters.
        ...
        --------
        >>> Solution().judgeCircle("UD")
        True   # Up then Down cancels out → back at origin

        >>> Solution().judgeCircle("LL")
        False  # Two left moves leave robot at (-2, 0)
        """

        # Initialize coordinates for the robot's position
        x, y = 0, 0

        # Count occurrences of each move and adjust coordinates accordingly
        # Left moves decrease x
        x = x - moves.count("L")
        # Right moves increase x
        x = x + moves.count("R")

        # Down moves decrease y
        y = y - moves.count("D")
        # Up moves increase y
        y = y + moves.count("U")
